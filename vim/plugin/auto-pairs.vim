...
function! AutoPairsInsert(key)
  ...
  let line = getline('.')
  let line_col = col('.')
  let prev_char = line[line_col-2]
  let current_char = line[line_col-1]
  let next_char = line[line_col]
  ...
  if !has_key(g:AutoPairs, a:key)
    ...
    if current_char == a:key
      " Not skip if not enought close brackets
      if a:key == ')' && len(split(line, '(', 1)) > len(split(line, ')', 1))
        return a:key
      endif
      return "\<Right>"
    end
  ...
  if open == close && prev_char =~ '\v\w'
    return a:key
  end

  if current_char != '' && current_char != ';' && !has_key(g:AutoPairsClosedPairs, current_char)
    return a:key
  endif
