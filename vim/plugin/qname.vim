...
if !exists("g:qname_buff_hotkey") || g:qname_buff_hotkey == ""
	let g:qname_buff_hotkey = "<F9>"
endif
...
exe "nmap" g:qname_buff_hotkey ":cal QNameInit(3)<cr>:~"
exe "imap" g:qname_buff_hotkey "<Esc>:cal QNameInit(3)<cr>:~"
exe "vmap" g:qname_buff_hotkey "<Esc>:cal QNameInit(3)<cr>:~"
let s:qname_base_hotkey = eval('"\'.g:qname_base_hotkey.'"')
let s:qname_func_hotkey = eval('"\'.g:qname_func_hotkey.'"')
let s:qname_buff_hotkey = eval('"\'.g:qname_buff_hotkey.'"')
let s:last_match = 0
let s:need_build = 0
...
function! QNameRun()
  ...
  while !empty(_key)
    ...
    elseif _key == "\<Right>"
      cal s:colPrinter.horz(1)
      cal s:act(0)
    elseif _key == s:qname_base_hotkey || _key == s:qname_func_hotkey || _key == s:qname_buff_hotkey
      cal QNameInit(0)
      break
    elseif _key == "\<F5>"
      unlet g:QNAME_LS
...
function! QNameInit(type)
    ...
    "endif
		...
		cnoremap <silent> ~ exe "cunmap \x7E"<CR>
    if s:last_match
